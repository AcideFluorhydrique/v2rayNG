...
data class SubscriptionUpdateResult(
    ...
    val skipCount: Int = 0,        // Subscriptions skipped (disabled, or not connected in the background)
    // Failed through the proxy; the user may retry these directly (FetchRoutePolicy)
    ...
    operator fun plus(other: SubscriptionUpdateResult): SubscriptionUpdateResult {
        return SubscriptionUpdateResult(
            ...
            directRetrySubIds = this.directRetrySubIds + other.directRetrySubIds
        )
    }
}
